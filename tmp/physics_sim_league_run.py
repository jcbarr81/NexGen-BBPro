...
def _summarize(
    ...
    metrics = {
        ...
        "errors_per_team_game": totals.get("e", 0) / (games * 2) if games else 0.0,
        "fielding_errors_per_team_game": totals.get("e_field", 0) / (games * 2) if games else 0.0,
        "throwing_errors_per_team_game": totals.get("e_throw", 0) / (games * 2) if games else 0.0,
        "gidp_per_team_game": totals.get("gidp", 0) / (games * 2) if games else 0.0,
        "fc_per_team_game": totals.get("fc", 0) / (games * 2) if games else 0.0,
    }
    ...
    for key, value in metrics.items():
        ...
        if key in benchmarks:
            deltas[key] = value - benchmarks[bench_key]
...
def run_sim(games_per_team: int, seed: int, players_path: Path) -> dict[str, float]:
    base_dir = BASE_DIR
    teams = _team_ids()
...
def main() -> None:
    ...
    parser.add_argument(
        "--players",
        type=Path,
        default=BASE_DIR / "data" / "players.csv",
    )
    args = parser.parse_args()
    summary = run_sim(args.games, args.seed, args.players)
